...
class Legoenv(Env):
    def __init__(self, max_tensors=6):
        ...
        self.actions = np.zeros(self.num_tensor_types * self.max_tensors + self.num_leg_combinations + 1, dtype=bool)
        self.actions[-1]=True
        self.actions_valid=[]
        self.current_active_tensor_index = 0
    ...
    def step(self, action):
        info = {'debug': []}
        #self.actions[action]=True  # for debugging purposes
        print(self.actions)
        done = False
        terminated = False
        ...
        if action < num_tensor_actions:
            ...
            if self.state[tensor_idx] != 0 or (tensor_idx != 0 and self.state[tensor_idx -1] == 0):
                ...
                self.actions[tensor_idx]=False
                if tensor_idx !=num_tensor_actions - 1:
                    self.actions[tensor_idx + self.num_tensor_types]=True
                #     self.current_active_tensor_index=tensor_idx + self.num_tensor_types
                # else:
                #     self.current_active_tensor_index=tensor_idx + self.num_tensor_types
                #これはtensor_typesが1出ないときに修正する必要があるかも
